...
def test_spi():
    cpol = 0
    cpha = 0
    clk, mosi, cs = zip(*list(spi_synth([3, 4, 5, 240], 8, 1.0, cpol, cpha, True, 2.0, 0.0)))
    ...
    if use_edges:
        ...
        #clk_s = list(noisify(sample_edge_list(iter(clk), sr), 15))
        mosi_s = list(noisify(sample_edge_list(iter(mosi), sr)))
        cs_s = list(noisify(sample_edge_list(iter(cs), sr)))
        
        #print('                  SEL >>> ', cs_s[-1])
        
        records = spi_decode_2(iter(clk_s), iter(mosi_s), iter(cs_s), cpol=cpol, cpha=cpha, lsb_first=True)
...
def test_uart():
    ...
    edges = uart_synth(bytearray('Hello world???'), 8, 115200.0, parity='even', idle_start=100.0e-7)
